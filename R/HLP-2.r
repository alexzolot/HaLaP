...
#' @title dett
 dett= function(patt= '^pac') catf('\ndetach("%s",  character.only = TRUE)', grep(patt, search(), v=TRUE))  ## prepare detach redundant packages
#' system time + sound
#' @title st
...
#' wrapper for shell + start explorer (win only)
#' @title expl
...
#' @title lss
 lss= function(verb=TRUE){ #object.sizes <-
...
#									round(object.size(get(object.name))/1048576,2)) ))
	...
	llss= (sort(sapply(ls(envir=.GlobalEnv), function (object.name){x= 1e-11
									try({x=round(object.size(get(object.name))/1048576, 2) },s=TRUE); x
								})))
...
#' @title lsDF
 lsDF= function(.all=FALSE, ...){ b= mdply(ls(envir= .GlobalEnv, ... ), function(a){if(.all)catt(a); aa=get(a); b=df(); cl<- substr(class(aa)[1],1,4)
...
#' @title libra
 libra= function(libs, verb=TRUE){
	...
	if(le(misspk) > 0) {install.packages(misspk, repos= "http://cran.stat.ucla.edu/", dependencies= TRUE)}
...
#' @title loo
 loo= function(patt='.RData'){gw(); 
	ff= dir(patt=patt, all.files =TRUE)
	if(le(ff) > 0) { 
...
#' save & remove
#' @examples 
#' ca= cars; srm(ca); ls(pattern='ca'); dir(patt='ca')	# creates ca.RData
